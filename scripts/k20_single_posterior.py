...
def k20_single_objective(config_file):
    ...
    plt.savefig(os.path.join(fig_dir, f'target_function_mean_{target}.png'), bbox_inches='tight')
    plt.clf()
    ...
    for query_idx in range(task['budget']):
        ...
        plt.plot((response.min(), response.max()), (response.min(), response.max()), linestyle='--', color='k')
