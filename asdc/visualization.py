...
def mpltern_scatter(
    ...
    if not tripcolor:
        s = ax.scatter(
            X.iloc[:,0], X.iloc[:,1], X.iloc[:,2], c=y,
            cmap=cmap, edgecolors='k', vmin=vlimits[0], vmax=vlimits[1]
        )
    else:
        s = ax.tripcolor(
            X.iloc[:,0], X.iloc[:,1], X.iloc[:,2], y,
            shading='flat', cmap=cmap, vmin=vlimits[0], vmax=vlimits[1]
        )
